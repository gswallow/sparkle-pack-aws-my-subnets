...
SfnRegistry.register(:public_subnet_id) do |offset = nil|
  offset.nil? ? public_subnets.map(&:subnet_id).sample : public_subnets.map(&:subnet_id).sort[offset]
end
...
SfnRegistry.register(:private_subnet_id) do |offset = nil|
  offset.nil? ? private_subnets.map(&:subnet_id).sample : private_subnets.map(&:subnet_id).sort[offset]
end
